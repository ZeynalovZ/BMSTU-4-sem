...
void MainWindow::on_download_clicked()
{
    ...
    str = BA.data();
    code_error = controller(&points, &edges, &n, &m, str, model_param, LOAD);
    if (code_error == OK)
    {
...
void MainWindow::on_rotate_clicked()
{
    ...
    model_param.angle = angle;
    if (ui->radio_X->isChecked())
    {
        code_error = controller(&points, &edges, &n, &m, str, model_param, ROTATE_X);
    }
    else if (ui->radio_Y->isChecked())
    {
         code_error = controller(&points, &edges, &n, &m, str, model_param, ROTATE_Y);
    }
    else if (ui->radio_Z->isChecked())
    {
         code_error = controller(&points, &edges, &n, &m, str, model_param, ROTATE_Z);
    }
    set_view(points, edges, n, m);
...
void MainWindow::on_transfer_clicked()
{
    ...
    code_error = controller(&points, &edges, &n, &m, str, model_param, TRANSFORM);
    set_view(points, edges, n, m);
}
...
void MainWindow::on_scale_clicked()
{
    ...
    model_param.ky = ky;
    model_param.kz = kz;
    code_error = controller(&points, &edges, &n, &m, str, model_param, SCALE);
    set_view(points, edges, n, m);
}
...
void MainWindow::on_upload_clicked()
{
    ...
    QByteArray BA = filename.toLatin1();
    str = BA.data();
    code_error = controller(&points, &edges, &n, &m, str, model_param, SAVE);
    if (code_error != OK)
    {
        QString message = "";
